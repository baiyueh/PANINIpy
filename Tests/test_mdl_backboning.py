...
def test_mdl_backboning():
    ...
    expected_global_compression_directed  = 0.7114680371853184
    expected_local_backbone_directed  = [(0, 3, 20), (1, 4, 3), (2, 1, 3), (3, 2, 3)]
    expected_local_compression_directed  = 0.8897699752517169
    ...
    assert pytest.approx(compression_global_directed , rel=1e-6) == expected_global_compression_directed , "Global Compression does not match expected result"
    assert pytest.approx(compression_local_directed , rel=1e-6) == expected_local_compression_directed , "Local Compression does not match expected result"

    # Compute backbones with undirected edges
    expected_global_backbone_undirected = [(0, 1, 12), (0, 4, 8), (0, 3, 20)]
    expected_global_compression_undirected = 0.7348799446767645
    expected_local_backbone_undirected = [(0, 1, 12), (0, 4, 8), (1, 2, 3), (0, 3, 20)]
    expected_local_compression_undirected = 0.8223220603580707
    ...
    assert pytest.approx(compression_global_undirected, rel=1e-6) == expected_global_compression_undirected, "Global Compression does not match expected result"
    assert pytest.approx(compression_local_undirected, rel=1e-6) == expected_local_compression_undirected, "Local Compression does not match expected result"
